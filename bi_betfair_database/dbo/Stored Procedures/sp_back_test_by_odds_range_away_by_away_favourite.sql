CREATE PROCEDURE [dbo].[sp_back_test_by_odds_range_away_by_away_favourite]
AS
BEGIN
...
FROM		dbo.event_odds			evo
			...
			AND dft.runner_name_display = 'Away'
...
SELECT		league_name
			...
			--,form_diff
FROM		#events
WHERE		favourite = 'Away favourite'
AND			[Away] >= 1.85
AND			[Away] <= 2.66
...
--SELECT		[Away]
--			,AVG(market_8hrs_before_order) AS market_8hrs_before_order
--			,SUM(bet_home_profit_loss) AS bet_home_profit_loss
--			,SUM(bet_draw_profit_loss) AS bet_draw_profit_loss
--			,SUM(bet_away_profit_loss) AS bet_away_profit_loss
--			,COUNT(*) AS bet_count
...
-- If Away odds are favourite
-- Bet on Away between >= 1.85
-- ###########################################################################
-- ###########################################################################
